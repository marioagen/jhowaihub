...
async function processQueue() {
    ...
    const { fileChunk, fileType, additionalData, headers, fileName, userEmail, tokenAzure, url, chunkIndex, totalChunks } = fileData.message;
...
async function uploadChunk(chunk, additionalData, headers, fileName, userEmail, tokenAzure, url, chunkIndex, totalChunks) {
    ...
    try {
        ...
        if (response.ok) {
            if (chunkIndex < totalChunks - 1) {
                self.postMessage({ type: 'uploadInProgress', success: true, nameFile: fileName, chunkIndex: chunkIndex + 1, chunks: totalChunks });
            } else if (chunkIndex === totalChunks - 1) {
                self.postMessage({ type: 'uploadComplete', success: true, nameFile: fileName, chunkIndex: chunkIndex + 1, chunks: totalChunks });
            }
            ...
            self.postMessage({ type: 'uploadComplete', success: false, nameFile: fileName, chunkIndex: chunkIndex + 1, chunks: totalChunks });
