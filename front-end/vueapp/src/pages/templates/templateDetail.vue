                                ...
                                <div class="mb-3">
                                    ...
                                    <Field name="name" rules="required|max:100" v-slot="{ field, errorMessage }">
                                        <input v-bind="field" type="text" class="form-control"
                                            :placeholder="$t('template.templateNamePlaceholder')" id="templateName"
                                            :class="{
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-3">
                                        ...
                                        <Field name="method" rules="required" v-slot="{ field, errorMessage }">
                                            <select v-bind="field" class="form-select" id="method" :class="{
                                                'is-invalid': errorMessage,
                                            }">
                                                <option v-for="method in methodsList" :key="method.id"
                                                    :value="method.value">
                                                    {{ method.value }}
                                                </option>
                                            </select>
                                    ...
                                    <div class="col-md-9">
                                        ...
                                        <Field name="url" rules="required|max:500" v-slot="{ field, errorMessage }">
                                            <input v-bind="field" type="text" class="form-control"
                                                :placeholder="$t('template.endpointUrlPlaceholder')" id="endpointUrl"
                                                maxlength="500" :class="{
                                                    'is-invalid': errorMessage,
                                                }" />
                                            <span class="validation-message text-danger" v-if="errorMessage">
                                ...
                                <ul class="nav nav-tabs mb-3" role="tablist">
                                    ...
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link" id="headers-tab" data-bs-toggle="tab"
                                            data-bs-target="#headers" type="button" role="tab">
                                            {{ $t("template.headers") }}
                                        </button>
                                    </li>
                                </ul>
                                <div class="tab-content">
                                    <div class="tab-pane fade show active" id="query-params" role="tabpanel">
                                        ...
                                        <div v-if="form.queryParams.length > 0">
                                            <div v-for="(param, index) in form.queryParams" :key="index"
                            ...
                            <div class="card-body">
                                ...
                                <Field name="body" rules="jsonValidation" v-slot="{ field, errorMessage }">
                                    <div class="position-relative">
                                        <textarea v-bind="field" ref="bodyTextarea" class="form-control font-monospace"
                                            rows="15" :placeholder="bodyPlaceholder" :class="{
                                                'is-invalid': errorMessage || jsonError,
                                            }" @input="handleBodyInput" @keydown="handleKeyDown"
                                            @blur="hideAutocomplete"></textarea>
                                        <div v-if="showAutocomplete" class="autocomplete-dropdown" :style="{
                                            ...
                                            <div v-for="(
option, index
                                                ) in filteredAutocompleteOptions" :key="index"
...
export default {
    ...
    methods: {
        ...
        loadToolsForAutocomplete() {
            ToolsService.getToolsList().then((tools) => {
                if (tools && Array.isArray(tools)) {
                    this.autocompleteOptions = tools.map((tool) => {
                        const translatedType = this.toolTypeDisplayName(tool.toolType);
                        const sanitizedType = textHelper.sanitizeToolNameForVariable(tool.toolType);
                        return {
