        ...
        <div class="container-fluid scroll-area mx-2">
            <div class="row align-items-center">
                <div class="col-10">
                    <div class="row">
                        <div class="col-auto">
                            <button class="btn btn-outline-primary btn-table btn-sm" @click="redirectToIndex"
                                type="button">
            ...
            <VueFlowComponent v-if="step !== null" :isEdit="isEdit" :stepId="stepId" :step="step" :stepOrder="stepOrder"
                @openNodeConfig="openNodeConfig" ref="VueflowComponent" :hasStepTools="hasStepTools" />
            ...
            <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel"
                ...
                <div class="offcanvas-body">
                    ...
                    <DependencySelector :previousStepTools="previousStepTools
                        " v-model="selectedDependencies" ref="dependencyTools" />
                    <hr />
                    <div v-if="isN8NTool" class="mb-3">
                        <select class="form-select form-select-sm w-auto mb-3" v-model="connector"
                            ...
                            <option v-for="connector in connectors" :key="connector.id" :value="connector.webhookId">
                                {{ connector.name }}
                            </option>
                        </select>
                        <div v-for="field in formFields" :key="field.name">
                            ...
                            <div v-else-if="
                                field.type === 'boolean'
                            " class="form-check mb-3" :disabled="loadingWebhooks ||
                                loadingInputs
                                ">
                                <input class="form-check-input" type="checkbox" :id="field.name" v-model="formData[field.name]
                                ...
                                <div v-for="(item, index) in formData[field.name]" :key="index">
                                    <div class="mb-3" v-for="child in field.children" :key="child.name">
                                        <label v-if="
                                            child.label
                                        " :for="child.name
                                            " class="form-label" :disabled="loadingWebhooks ||
                                                loadingInputs
                                                ">
                                            {{
                                                child.label
                                        ...
                                        <label v-else :for="child.name
                                            " class="form-label text-capitalize">
                                            {{ child.name }}
                                        </label>
                                        <input :id="child.name" v-model="formData[
                                            field
                                                .name
                                        ][index][
                                            child
                            ...
                            ">
                                ...
                                <textarea class="form-control form-control-sm text-long" :id="field.name" v-model="formData[field.name]
                                    " rows="4" />
                            </div>
                        </div>

                        <div class="mt-4">
                            <button type="button" class="btn btn-primary" @click="updateNodeWithForm" :disabled="loadingWebhooks ||
                                loadingInputs
                                ">
                                {{ $t("common.save") }}
                            </button>
                    ...
                    <div v-else class="mb-3">
                        ...
                        <div class="background-div" v-for="(param, index) in parameters" :key="index">
                            <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" v-model="parameters[index].value
                                "></textarea>
                        </div>
    ...
    methods: {
        ...
        async openNodeConfig(nodes, selectedNode) {
            ...
            if (this.isTargetTool(ToolType.N8N)) {
                ...
                this.$router.push({ name: 'PromptSelector' });
                return;
