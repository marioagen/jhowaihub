    ...
    <main>
        <div class="container-fluid mt-4">
            <div>
                ...
                <div class="row">
                    <prompt-view :hashDocument="hashDocument" :historyListOrder="historyListOrder"
                        @showHistory="showHistory" @unshiftHistoryList="
                            unshiftHistoryList
                        " @pushHistoryList="pushHistoryList" @showAlertToast="showAlertToast"
                        @clearMyInterval="clearMyInterval" v-if="!isExpandedHistory" />

                    <doc-view @showNormalize="normalize" id="docView" v-if="
                        viewMode === 'doc' ||
                        viewMode === 'both'
                    " :documentView="viewMode" />
                    <div :id="'docHistory'" :class="viewMode === 'both'
                        ? 'col-md-6'
                        : 'col-12'
                        ">
                        <step-analysis-view :document-id="parseInt(idAnalyzer)
                            " :card-id="parseInt(idCard)" @show-alert-toast="
                                showAlertToast
                            " v-if="
                                viewMode === 'history' ||
                                viewMode === 'both'
            ...
            <div v-if="isExpandedHistory" style="position: absolute; top: 50%">
                <a class="btn btn-light btn-sm shadow" :title="$t('quizzes.questionnaireAndAi')
                    " @click="expandHistory">
...
import DocumentRejectionModal from "@/components/analyze/DocumentRejectionModal";
import DocumentViewRejectionModal from "@/components/analyze/DocumentViewRejectionModal";
