        ...
        <div class="mb-2 mt-1">
            <div class="div-quiz">
                <select class="form-select custom-input" id="selectText" v-model="quizSelected">
                    ...
                    <option :value="item" v-for="(item, index) in quizzes" :key="index">
                        {{ item.title }}
                    </option>
                </select>
                <a
                    ...
                    style="
                        margin: 0px 4.5px 0px 5px;
                        min-width: max-content;
                        box-shadow: none !important;
                    "
                    :title="$t('labelViewQuestions')"
                    v-if="quizSelected.id && !loadingApplying"
                    @click="openModal"
                >
                ...
                <a
                    ...
                    :title="$t('labelApplyQuestionnaire')"
                    v-if="quizSelected.id && !loadingApplying"
                    @click="sendQuiz"
                >
                ...
                <a
                    ...
                    style="
                        ...
                        box-shadow: none !important;
                    "
                    :title="$t('labelApplyingQuestionnaire')"
                    v-if="quizSelected.id && loadingApplying"
                >
        ...
        <div class="mb-1 mt-1">
            <div class="input-group custom-input-button">
                <input
                    ...
                    :placeholder="$t('labelAskTheAi')"
                    id="inputText"
                    v-model="input"
                    @keyup.enter="submitInput"
                />
                <i
                    class="input-group-text fas fa-arrow-circle-up custom-input"
                    :title="$t('labelSendQuestion')"
                    v-if="input.length > 0"
                    @click="submitInput"
                ></i>
            </div>
        ...
        <div class="mb-2">
            <div style="width: 100%">
                ...
                <div style="float: right" v-if="output != ''">
                    <a @click="copyToClipboard(output)">
                        <i
            ...
            sendQuiz: function () {
                ...
                api.post('/Document/inputQuestionnaire', paramsReq)
                    .then(function (response) {
                        ...
                        self.alertToast(
                            self.$t('labelQuestionnaireAppliedSuccessfully'),
                            'toast-success'
                        )
                    ...
                    .catch(function (e) {
                        ...
                        if (e.response && e.response.data === 'No Credits to send a Question') {
                            self.alertToast(
                                self.$t('labelNumberOfQuestionsHasBeenExceeded'),
                                'toast-danger'
                            )
                        } else if (e.response.status === 404) {
                            self.alertToast(
                                self.$t('labelAnInconsistencyWasIdentifiedInTheDocument'),
                                'toast-danger'
                            )
                            ...
                            self.alertToast(
                                self.$t('labelFailedToApplyQuestionnaire'),
                                'toast-danger'
                            )
                        }
                    })
