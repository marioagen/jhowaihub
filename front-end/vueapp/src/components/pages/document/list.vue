    ...
    <main>
        <div class="container-fluid scroll-area mx-2">
            <div class="mt-3 mb-3">
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <div class="flex-grow-1">
                                <SearchComponent :entity="entitySearch" :resetInput="resetInputSearch"
                                    @search="filterList" />
                            </div>
                ...
                <div class="mb-3">
                    <button type="button" class="btn delete-custom d-flex align-items-center"
                        @click="confirmationDialog(item)" v-if="this.listIds.length > 0">
                        ...
                        <table class="table table-bordered-vertical table-striped">
                            <tbody>
                                <tr class="tr-head-1">
                                    ...
                                    <td class="content-left-middle">
                                        {{ $t("labelDescription") }}
                                        <i id="1" class="fas fa-sort" @click="orderList(1)" style="cursor: pointer"
                                            :title="$t('labelOrder')"></i>
                                    </td>
                                    <td class="content-left-middle">
                                        {{ $t("labelInclusionDate") }}
                                        <i id="2" class="fas fa-sort" @click="orderList(2)" style="cursor: pointer"
                                            :title="$t('labelOrder')"></i>
                                    </td>
                                    <td class="content-left-middle">
                                        {{ $t("labelStatus") }}
                                        <i id="3" class="fas fa-sort" @click="orderList(3)" style="cursor: pointer"
                                            :title="$t('labelOrder')"></i>
                                    </td>
                                    <td class="content-left-middle">
                                        {{ $t("labelTeams") }}
                                ...
                                <tr v-for="(item, index) in dataDocument" :key="index">
                                        ...
                                        <a>
                                            <div class="form-check">
                                                <input class="form-check-input checkbox" type="checkbox" value=""
                                                    :id="item.id" @click="countChecks(item.id)" />
                                            </div>
                                        </a>
                                    </td>
                                    ...
                                    <td class="content-right-middle" style="width: 100px">
                                        <button class="btn btn-primary btn-sm" :title="$t('labelAnalyze')"
                                            @click="embeddingData(item.id)" v-if="item.status == 0">
                                        ...
                                        }" :title="$t('labelConsult')" v-if="item.status == 1">
            ...
            <div class="row mt-1" v-if="!loading && dataDocument.length > 0">
                <div class="col">
                    ...
                    <div class="d-inline-block" style="margin-left: 1%">
                        <select class="form-select form-select-sm d-inline" v-model="selectedOption"
                    ...
                    <nav>
                        <ul class="pagination justify-content-center">
                            ...
                            <li :class="pagination.currentPage === i ? `page-item active` : `page-item`"
                                ...
                                <a class="page-link" @click="getList({ search: '', page: i, type: null })"
            ...
            <modal-alert v-if="modalAlertShow" :type="'Confirm'" :entity="modalEntity"
                :alertTitle="$t('labelYouAreAboutToDeleteDocument')" :alertMessage="$t('labelThisActionCannotBeUndone')"
                :okLabel="$t('labelConfirm')" :cancelLabel="$t('labelCancel')" @open="deleteItem" @close="closeModal" />
            <modal-alert v-if="modalWarningShow" :type="'Warning'" :entity="modalEntity"
                :alertTitle="$t('labelCaution')" :alertMessage="$t('labelNotReloadThePage')"
                :okLabel="$t('labelConfirm')" @open="confirmWarning" @close="closeModalWarning" />
            <NormalizeIndex :docData="docDataEmbedding" :isReprocessing="isReprocessing" v-if="showLoading">
            </NormalizeIndex>
        </div>
    </main>
</template>
