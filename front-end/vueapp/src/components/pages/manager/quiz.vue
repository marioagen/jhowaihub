<template>
    <main>
        <div class="container-fluid mt-4">
            <div class="custom-padding">
                ...
                <!-- Component SearchBar -->
                <search-bar :entity="entitySearch" :resetInput="resetInputSearch" @search="getList" @action="redirectToNewQuiz" />
                ...
                <div class="row" v-if="dataQuiz.length === 0 && !loading && (searching || !searching)">
                    <div class="table-responsive">
                        <table class="table table-striped">
                            <tbody>
                                <tr class="tr-head-1">
                                    <td style="text-align: center;">
                                        <i class="fas fa-exclamation-circle text-secondary"></i>&nbsp;{{ $t('labelNoQuestionnairesWereFound') }}.
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                ...
                <div class="row scroll-area" v-if="!loading && dataQuiz.length > 0">
                    <div class="table-responsive">
                        <table class="table table-bordered-vertical table-striped">
                            <tbody>
                                <tr class="tr-head-1">
                                    ...
                                    <td class="content-left-middle">{{ $t('labelName') }} <i id="2" class="fas fa-sort" @click="orderList(2)" style="cursor: pointer;" :title="$t('labelOrder')"></i></td>
                                    <td class="content-center-middle">{{ $t('labelTypeDoc') }} <i id="3" class="fas fa-sort" @click="orderList(3)" style="cursor: pointer;" :title="$t('labelOrder')"></i></td>
                                ...
                                <tr v-for="(item, index) in dataQuiz" :key="index">
                                    ...
                                    <td class="content-center-middle" style="width: 51px;"> {{ item.id }} </td>
                                    <td class="content-left-middle">
                                        <truncate-text :item="item" :text="item.title"/>
                                    </td>
                                    <td class="content-center-middle" style="width: 175px;"> 
                                        <span v-html="setBreakLine(item.typeDoc.name)"></span> 
                                    </td>
                                    <td class="content-center-middle" style="width: 164px;"> {{ item.questions.length }} </td>
                                    ...
                                    <td class="content-right-middle" style="width: 51px;" v-if="item.emailCreator === $store.state.userProfile.login">
                                        <router-link class="btn btn-success btn-sm" :to="{ name: 'QuizEdit', params: { id: item.id }, query: { page: pagination.currentPage } }" :title="$t('labelEdit')">
                ...
                <div class="row mt-1" v-if="!loading && dataQuiz.length > 0">
                    <div class="col">
                        ...
                        <Pagination :paginationData="pagination" :dataList="dataQuiz"></Pagination>
                    </div>
                    <div class="col-auto">
    ...
    <modal-alert v-if="modalAlertShow" :type="'Confirm'" :entity="modalEntity" :alertTitle="$t('labelYouAreAboutToDeleteQuestionnaire')" :alertMessage="$t('labelThisActionCannotBeUndone')" :okLabel="$t('labelConfirm')" :cancelLabel="$t('labelCancel')" @open="deleteItem" @close="closeModal" />
</template>
    ...
    export default {
        ...
        methods: {
            ...
            setCrumbsData: function () {
                this.crumbsData = [
                    { crumb: this.$t('labelManage'), link: { to: 'Quiz' } },
                    { crumb: this.$t('labelQuestionnaires'), link: { to: 'Quiz', queryPage: this.$route.query.page } },
                ];
            },
            setEntitySearch: function () {
                this.entitySearch = {
                    screen: "quiz",
                    labelInput: this.$t('labelSearchQuestionnaire'),
                    placeholderInput: this.$t('labelQuestionnaireSearch'),
                    labelButton: this.$t('labelNewQuestionnaire'),
                };
            },
            getList: function (obj) { // obj = { search, page, type }
                ...
                let self = this;
                api.get('/Questionnaire/Paged', { params: paramsReq })
                    .then(function (response) { // Handle success
                        self.dataQuiz = response.data.content;
                        self.pagination = {
            ...
            deleteItem: function (id) {
                let self = this;
                api.delete('/Questionnaire/DeleteByIds', { data: this.listIds })
                    .then(function (response) { // Handle success
                        self.closeModal();
            ...
            redirectToNewQuiz: function (quiz) {
