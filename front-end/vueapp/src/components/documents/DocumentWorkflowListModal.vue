<template>
  <ModalComponent id="documentWorkflowListModal" :isLoading="isLoading" ref="modal" :hideFooter="true">
    <template #header>
    ...
    <template #body>
      <div class="px-2">
        ...
        <div class="input-group mb-3 search-container">
          ...
          <input type="text" class="form-control border-start-0 border-end-0 ps-0 search-input"
            :class="{ 'params-filled': searchData.search }" :placeholder="$t('documents.workflowListModal.searchPlaceholder')" v-model="searchData.search"
            @input="filterData" />
          <span class="input-group-text bg-white border-start-0" v-if="searchData.search" @click="cleanInput"
            style="cursor: pointer;">
            <LucideIcon icon="X" size="18" class="text-muted" />
          </span>
          <span class="input-group-text bg-white border-start-0" v-else>
          </span>
        </div>

        <!-- Workflow List -->
        <div class="workflow-list d-flex flex-column gap-3">
          <div v-for="workflow in workflowListByDocument" :key="workflow.id"
            ...
            <div class="d-flex align-items-center gap-3">
              <div class="icon-circle d-flex align-items-center justify-content-center">
                <LucideIcon icon="Workflow" size="20" class="text-primary" />
              </div>
              <div class="workflow-info">
                <h6 class="mb-0 fw-normal text-dark">{{ workflow.name }}</h6>
                <small class="text-muted">{{ $t("documents.workflowListModal.clickToView") }}</small>
              </div>
            </div>
            <div class="arrow-icon">
              <LucideIcon icon="ChevronRight" size="20" class="text-muted" />
            </div>
          </div>

          <div v-if="workflowListByDocument.length === 0 && !isLoading" class="text-center py-4 text-muted">
    ...
    <template #footer>
      <div class="w-100 d-flex justify-content-end py-2 px-2 border-top mt-3">
        <button class="btn btn-outline-secondary btn-sm px-4" @click="close">
