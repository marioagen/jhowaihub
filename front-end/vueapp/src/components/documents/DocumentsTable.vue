<template>
    ...
    <div v-if="showTable">
        <TableComponent modalName="documents.title" emptyMessage="documents.notFound" :data="table.data"
            :columns="table.columns" :isLoading="table.isLoading" :pagination="table.pagination"
            @selectedRows="selectedRows" @change-page="changePage">
            ...
            <template #cell-workflows="{ data }">
                <BadgeOutlinedComponent v-for="(workflowData, index) in data.row.workflowProgress" :key="index"
                    :text="`${workflowData.workflowName} (${workflowData.currentStep}/${workflowData.totalSteps})`"
                    :clickable="false" class="ms-1" />
            </template>
