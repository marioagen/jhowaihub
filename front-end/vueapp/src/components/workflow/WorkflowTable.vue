<template>
        ...
        <TableComponent modalName="workflow.index" emptyMessage="workflow.notFound" :data="table.data"
            ...
            <template #cell-actions="{ data }">
                <ActionTableListComponent v-slot="{ actionClass }">
                    <a :class="actionClass" class="text-primary" @click="redirectToIndex(data.row)"
                    ...
                    <a :class="actionClass" class="text-primary" @click="openCloneModal(data.row)"
                        v-tooltip="$t('workflow.clone')">
                        <LucideIcon icon="Copy" />
                    </a>
                    <a :class="actionClass" class="text-danger" style="color: red" @click="openConfirmation(data.row)"
    ...
    <ConfirmModal id="deleteConfirm" title="documents.deleteValidationTitle" message="common.thisActionCannotBeUndone"
        cancelText="common.cancel" confirmText="common.confirm" confirmVariant="danger" ref="DeleteDialog"
        :isLoading="isDeleting" @confirm="deleteWorkflow" />
    <ConfirmModalValidationInput id="deleteValidationConfirm" title="documents.deleteValidationTitle"
        :message="deleteValidationMessage" cancelText="common.cancel" confirmText="documents.confirmPermanentDelete"
        :placeholder="$t('documents.deleteValidationPlaceholder', { name: selectedWorkflowName })"
        :validationText="selectedWorkflowName" confirmVariant="danger" iconeName="Trash2" iconVariant="danger"
        ref="DeleteValidationDialog" :isLoading="isDeleting" @confirm="deleteWorkflow" />
    <ModalComponent id="cloneWorkflowModal" ref="CloneModal" :title="'workflow.cloneTitle'"
        :saveText="'workflow.cloneConfirm'" :isLoading="isCloning" @save="confirmClone" @cancel="closeCloneModal">
        <template #body>
            <div class="modal-body">
                <div class="mb-3">
                    <label for="cloneWorkflowName" class="form-label">
                        {{ $t("workflow.cloneNameLabel") }}
                    </label>
                    <input id="cloneWorkflowName" v-model="cloneWorkflowName" type="text" class="form-control"
                        :placeholder="$t('workflow.namePlaceholder')" @keyup.enter="confirmClone" />
                </div>
            </div>
        </template>
    </ModalComponent>
