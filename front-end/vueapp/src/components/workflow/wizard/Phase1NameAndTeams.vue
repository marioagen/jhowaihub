    ...
    <div class="phase-container">
        ...
        <div class="row">
            <div class="col-12">
                ...
                <Field name="name" rules="required" v-slot="{ field, errorMessage }" v-model="workflowData.name">
                    <input class="form-control form-control-sm" :placeholder="$t('workflow.namePlaceholder')"
                        v-bind="field" :value="workflowData.name" />
                ...
                <div v-else>
                    <div class="mb-2">
                        <input type="text" class="form-control form-control-sm"
                            :placeholder="$t('management.teams.searchTeams')" v-model="searchTeam" />
                    </div>
                        ...
                        <div class="row">
                            <div v-for="team in filteredTeams" :key="team.id" class="col-3 p-1">
                                <div class="form-check d-flex align-items-center">
                                    <input class="form-check-input me-3" type="checkbox" :id="`team-${team.id}`"
                                        :value="team.id" v-model="selectedTeams" />
                                    <label class="form-check-label fw-semibold" :for="`team-${team.id}`">
                                        {{ team.text }}
                                    </label>
                                </div>
                            </div>
