        ...
        <div class="card-content">
            <div class="card-body pb-0">
                <div class="mb-1 title-text">
                    ...
                    <div v-if="isCardRejected" class="badge fr flex-shrink-1 mt-1"
                        :style="badgeStyle(dataCard.status.color)">
                        {{ $t("workflow.statusList." + dataCard.status.name.toLowerCase()) }}
                    </div>
                    <div v-else class="badge fr flex-shrink-1 mt-1" :style="badgeStyle(dataStep.status.color)">
                ...
                " class="mb-2">
                    ...
                    <button type="button" @click.stop="unassignUser" class="btn btn-sm btn-unlink ms-1 px-1"
                        v-tooltip.right="$t('card.unassignInfo')
                            ">
                        <LucideIcon v-if="isUnassigningUser" icon="Loader" :size="16"
                ...
                <div class="mb-2 d-flex justify-content-between align-items-center flex-wrap" v-if="!showLoading">
                    <div>
                        <button type="button" class="btn btn-sm btn-primary" @click="redirectToAnalyzer">
                            {{ $t("common.analyze") }}
                            <LucideIcon icon="FileClock" :size="15" class="ms-2" />
                        </button>
                    </div>
                    <div v-if="!isLastStep">
                        <button v-if="
                            !isFirstStep ||
                            dataCard.assignedUser
                        " class="btn btn-sm btn-outline-primary float-end" @click.stop="advanceStep">
                            <span>
                                {{ $t("common.advance") }}
                            </span>
                        ...
                        ">
                            <div v-if="isAdmin" class="btn-group">
                                <button type="button"
                                    ...
                                    " icon="Loader" :size="16" class="mr-2 animate-spin text-white" />
                                    <span>
                                        {{
                                            $t(
                                                "card.assignBtn"
                                ...
                                <ul class="dropdown-menu p-2 users-list">
                                    ...
                                    " class="mb-1">
                                        <div class="input-group input-group-sm">
                                            ...
                                            <input :id="`filter-user-${dataCard.id}`" v-model="userSearchText
                                                " type="text" name="filter" class="form-control" @input="
                                                    searchUser
                                                " @click.stop="" />
                                        </div>
                                    </li>
                                    <li v-for="user in filteredUsers" :key="user.id" @click.stop="
                                        assignUser(
                                            user.id
                                        )
                                        ">
                            ...
                            <button v-else type="button" class="btn btn-sm btn-primary assing-btn" @click.stop="
                                assignUser(loggedUserId)
                                ">
                                <LucideIcon v-if="
                                    isUpdatingAssignedUser
                                " icon="Loader" :size="12" class="mr-2 animate-spin text-white" />
                                {{ $t("card.assignBtn") }}
