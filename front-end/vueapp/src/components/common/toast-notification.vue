        ...
        >
            ...
            <div class="toast-body toast-upload" v-if="!isFinished">
                ...
                <div class="scroll-area">
                    <div
                        :class="['extra-content', { show: isExpanded }]"
                        v-for="(item, index) in dataUpload"
                    >
                        <div class="row">
    ...
    export default {
        ...
        methods: {
            ...
            handleUploadComplete(payload) {
                this.dataUpload.forEach((upload) => {
                    if (payload.nameFile === upload.nameFile) {
                        if (
                            upload.success &&
                            (payload.chunks == 1 || payload.chunks === upload.countProgress + 1)
                        ) {
                ...
                if (this.dataUpload.every((item) => item.isLoading === false && item.success)) {
                    ...
                    this.dataUpload.every((item) => item.isLoading === false && !item.success)
                ) {
            ...
            handleUploadInProgress(payload) {
                ...
                this.uploadingFiles =
                    this.maxSizeUpload - this.dataUpload.filter((item) => item.isLoading).length
            },
