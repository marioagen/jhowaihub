<template>
    <div class="row">
        <div class="col-lg-4">
            <div class="mb-4">
                <DependencySelector :previousStepTools="previousStepTools" :selectedDependencies="selectedDependencies"
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-3">
                            ...
                            <select :value="templateData.method" class="form-select" id="method"
                                :disabled="readOnly || editable">
                                <option v-for="method in methodsList" :key="method.id" :value="method.value">
                                    {{ method.value }}
                                </option>
                            </select>
                        </div>
                        <div class="col-md-9">
                            ...
                            <input :value="templateData.url" @input="updateUrl($event.target.value)" type="text"
                                class="form-control" id="endpointUrl" maxlength="500" :disabled="readOnly" />
                        </div>
                    </div>
                    <div class="row">
                        <ul class="nav nav-tabs mb-3" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active" id="query-params-tab" data-bs-toggle="tab"
                                    data-bs-target="#query-params" type="button" role="tab">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade show active" id="query-params" role="tabpanel">
                                ...
                                <div v-if="templateData.queryParams.length > 0">
                                    <div v-for="(param, index) in templateData.queryParams" :key="index"
                                        class="row mb-2 align-items-center">
                                        <div :class="editable ? 'col-6' : 'col-12'">
                                            <input :value="param.key" type="text" class="form-control form-control-sm"
                                                disabled :placeholder="$t('template.keyPlaceholder')" />
                                        </div>
                                        <div v-if="editable" class="col-6">
                                            <input :value="param.value"
                                                @input="updateQueryParam(index, $event.target.value)" type="text"
                            ...
                            <div class="tab-pane fade" id="headers" role="tabpanel">
                                ...
                                <div v-if="templateData.headers.length > 0">
                                    <div v-for="(header, index) in templateData.headers" :key="index"
                                        class="row mb-2 align-items-center">
                                        <div :class="editable ? 'col-6' : 'col-12'">
                                            <input :value="header.key" type="text" class="form-control form-control-sm"
                                                disabled :placeholder="$t('template.keyPlaceholder')" />
                                        </div>
                                        <div v-if="editable" class="col-6">
                                            <input :value="header.value"
                                                @input="updateHeader(index, $event.target.value)" type="text"
                                                class="form-control form-control-sm"
                                                :placeholder="$t('template.valuePlaceholder')" />
                                        </div>
                                    </div>
                ...
                <div class="card-body">
                    ...
                    <Field name="body" rules="jsonValidation" v-slot="{ field, errorMessage }">
                        <div class="position-relative">
                            <textarea :name="field.name" :value="templateData.body"
                                @input="handleBodyInput($event); field.onInput($event)"
                                @blur="handleBodyBlur($event); field.onBlur($event)" class="form-control font-monospace"
                                rows="17" :disabled="readOnly && !editable"
