    ...
    _setActiveIndicatorElement(element) {
        if (this._indicatorsElement) {
            ...
            for (let i = 0; i < indicators.length; i++) {
                if (
                    Number.parseInt(indicators[i].getAttribute("data-bs-slide-to"), 10) === this._getItemIndex(element)
                ) {
...
class Tooltip extends BaseComponent {
    ...
    getTitle() {
        ...
        if (!title) {
            title =
                typeof this._config.title === "function" ? this._config.title.call(this._element) : this._config.title;
        }
