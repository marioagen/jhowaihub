...
namespace WoopiAiHub.Repository
{
    public class DocumentRepository : IDocumentRepository
    {
        ...
        {
            var documents = _context.Documents.Where(a => a.Id.Equals(id) && a.Status.Equals(Domain.Enum.Status.NotAnalyzed));
            if (documents.Any())
            {
                documents.ExecuteUpdate(b => b
                .SetProperty(u => u.Status, Domain.Enum.Status.Analyzed));
                _context.SaveChanges();
                return true;
            }
            else
