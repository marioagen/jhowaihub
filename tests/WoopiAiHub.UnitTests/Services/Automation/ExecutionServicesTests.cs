    ...
    public class ExecutionServicesTests
    {
        ...
        public async Task HandleExecutionProgress_CardWithoutBatch_ShouldUpdateStatusAndNotify()
        {
            ...
            _mocker.GetMock<IHubNotifier>()
                .Setup(x => x.CardProgessAsync(email, card.Id, It.IsAny<double>(), stepTool1.StepId, It.IsAny<string>()))
                .Returns(Task.CompletedTask);
            ...
            _mocker.GetMock<IHubNotifier>().Verify(x => x.CardProgessAsync(email, card.Id, 50.0, stepTool1.StepId, tool.Name), Times.Once);
        }
        ...
        public async Task HandleExecutionProgress_CardWithBatch_ShouldNotifyAllBatchCards()
        {
            ...
            _mocker.GetMock<IHubNotifier>()
                .Setup(x => x.CardProgessAsync(email, It.IsAny<int>(), It.IsAny<double>(), stepTool.StepId, It.IsAny<string>()))
                .Returns(Task.CompletedTask);
            ...
            _mocker.GetMock<IHubNotifier>().Verify(x => x.CardProgessAsync(email, batchCard1.Id, 100.0, stepTool.StepId, string.Empty), Times.Once);
            _mocker.GetMock<IHubNotifier>().Verify(x => x.CardProgessAsync(email, batchCard2.Id, 100.0, stepTool.StepId, string.Empty), Times.Once);
        }
        ...
        public async Task HandleExecutionProgress_CardNotFound_ShouldReturnEarly()
        {
            ...
            _mocker.GetMock<IHubNotifier>().Verify(x => x.CardProgessAsync(It.IsAny<string>(), It.IsAny<int>(), It.IsAny<double>(), It.IsAny<int>(), It.IsAny<string>()), Times.Never);
        }
        ...
        public async Task HandleExecutionProgress_NoStepTools_ShouldNotify100Percent()
        {
            ...
            _mocker.GetMock<IHubNotifier>()
                .Setup(x => x.CardProgessAsync(email, card.Id, 100.0, stepTool.StepId, string.Empty))
                .Returns(Task.CompletedTask);
            ...
            // Assert
            _mocker.GetMock<IHubNotifier>().Verify(x => x.CardProgessAsync(email, card.Id, 100.0, stepTool.StepId, string.Empty), Times.Once);
        }
        ...
        public async Task HandleExecutionProgress_WithRunningTool_ShouldIdentifyToolName()
        {
            ...
            _mocker.GetMock<IHubNotifier>()
                .Setup(x => x.CardProgessAsync(email, card.Id, It.IsAny<double>(), stepTool1.StepId, "Running Tool Name"))
                .Returns(Task.CompletedTask);
            ...
            // Assert
            _mocker.GetMock<IHubNotifier>().Verify(x => x.CardProgessAsync(email, card.Id, 0.0, stepTool1.StepId, "Running Tool Name"), Times.Once);
        }
        ...
        public async Task HandleExecutionProgress_NullToolName_ShouldUseEmptyString()
        {
            ...
            // Assert
            _mocker.GetMock<IHubNotifier>().Verify(x => x.CardProgessAsync(email, card.Id, 0.0, stepTool.StepId, string.Empty), Times.Once);
        }
        ...
        public async Task HandleExecutionProgress_NullExecution_ShouldCountAsIncomplete()
        {
            ...
            _mocker.GetMock<IHubNotifier>().Verify(x => x.CardProgessAsync(email, card.Id, 50.0, stepTool1.StepId, string.Empty), Times.Once);
        }
        ...
        public async Task HandleExecutionProgress_AllStepToolsReady_ShouldNotify100Percent()
        {
            ...
            _mocker.GetMock<IHubNotifier>()
                .Setup(x => x.CardProgessAsync(email, card.Id, 100.0, stepTool1.StepId, string.Empty))
                .Returns(Task.CompletedTask);
            ...
            // Assert
            _mocker.GetMock<IHubNotifier>().Verify(x => x.CardProgessAsync(email, card.Id, 100.0, stepTool1.StepId, string.Empty), Times.Once);
        }
    }
}
